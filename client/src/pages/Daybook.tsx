...
export default function Daybook({ user }: { user?: any } = {}) {
  ...
  // Hide vouchers auto-generated by sales/POS/purchase/stock-transfer/closing
  // by default. The user only created vouchers manually now want to focus on
  // their own entries; system entries are still available behind a toggle.
  ...
  const { data: vouchers = [], isLoading } = useQuery<Voucher[]>({
    ...
    queryFn: async () => {
      ...
      if (!showSystemEntries) params.append("includeSystem", "false");
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="space-y-2 flex items-end">
              <div className="flex items-center gap-2 pb-2">
                <Switch
                  id="show-system-entries"
                  checked={showSystemEntries}
                  onCheckedChange={setShowSystemEntries}
                  data-testid="switch-show-system"
                />
                <Label htmlFor="show-system-entries" className="cursor-pointer">
                  Show automatic entries
                </Label>
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                {viewEntriesLoading ? (
                  ...
                  (() => {
                    ...
                    // Purchase items are stock items linked to a PO
                    const purchaseItems = viewVoucherEntries.filter(
                      (e: ViewVoucherEntry) => e.isPurchaseItem || e.isStockItem,
                    );

                    // When purchase items exist, only show charge-type ledger accounts (Freight, Fumigation, etc.)
                    // When no purchase items found (PO not linked), show ALL ledger entries so the dialog isn't empty
