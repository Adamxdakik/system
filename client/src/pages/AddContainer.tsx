...
export default function AddContainer() {
  ...
  const itemsTotal = watchItems.reduce((sum, item) => {
    return sum + ((item.quantity || 0) * (item.ratePerKg || 0));
  }, 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold">Add Container</h1>
          <p className="text-sm text-muted-foreground">
            Create a new container with purchase order items and charges
          </p>
        </div>
      </div>
                ...
                <table className="w-full min-w-[600px]">
                        ...
                        <tr key={field.id} className="border-t relative">
                          <td className="p-2">
                            <div className="relative">
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") {
                                    ...
                                    const filteredItems = getFilteredItems(
                                      stockItemSearch[index] || item?.itemName || "",
                                    );
                              ...
                              />
                              {stockItemSearch[index] &&
                                getFilteredItems(stockItemSearch[index]).length > 0 && (
                          ...
                          <td className="p-2">
                            <Input
                              {...form.register(`items.${index}.ratePerKg`, {
                                valueAsNumber: true,
                              })}
                              type="number"
                              step="0.01"
                  ...
                  <tfoot className="bg-muted/30">
                    <tr className="border-t">
                      <td colSpan={3} className="p-2 text-right font-medium text-sm">
                        Items Total:
                      </td>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end pt-2 border-t">
                    <span className="font-medium text-sm">
                      Charges Total: ${chargesTotal.toFixed(2)}
                    </span>
                  </div>
                </div>
              )}
