...
export default function VoucherEdit() {
  ...
  const allAccountsWithBalances: CombinedAccount[] = useMemo(() => {
    ...
    (allAccountsData as any[]).filter(a => a.type === "factorySupplier").forEach(fs => {
      ...
      const baseBalance = typeof fs.balance === "number" ? fs.balance : parseFloat(fs.balance || "0");
      const adjustedBalance = baseBalance + adjustment;
      accounts.push({
        type: "factorySupplier" as const,
        id: Number(fs.accountId || fs.id),
        name: fs.name,
  ...
  const findAccountDetails = (entry: VoucherEntry) => {
    if (entry.ledgerAccountId) {
      ...
      return {
        ...
        name: account?.name || `Ledger #${entry.ledgerAccountId}`,
      };
      ...
      return {
        ...
        name: account?.bankName || `Bank #${entry.bankAccountId}`,
      };
    } else if (entry.factorySupplierId) {
      const fs = (allAccountsData as any[]).find(
        a => a.type === "factorySupplier" && Number(a.accountId || a.id) === entry.factorySupplierId
      );
      return {
        type: "factorySupplier" as const,
      ...
      return {
        type: "supplier" as const,
        id: entry.supplierId,
        name: supplier?.legalName || `Supplier #${entry.supplierId}`,
      };
  ...
  useEffect(() => {
    ...
    if (isPaymentOrReceipt && voucher.entries.length >= 1) {
      // Smart source detection: prefer non-liability (bank/ledger) as the pay-from/receive-into account.
      // For Payment: source is CREDITED (money leaves). For Receipt: source is DEBITED (money arrives).
      const paymentEntry =
        voucher.entries.find((e: VoucherEntry) => {
        ...
        voucher.entries.find((e: VoucherEntry) => {
          // Fallback: liability account as source (e.g. advance payments)
          const cr = parseFloat(e.creditAmount || "0");
          const dr = parseFloat(e.debitAmount || "0");
          const isLiability = !!(e.supplierId || e.factorySupplierId || e.employeeId);
          if (!isLiability) return false;
      ...
      const isLiabilityPayment = !!(paymentEntry.supplierId || paymentEntry.factorySupplierId || paymentEntry.employeeId);

      // Destination entries: all entries except source, with the opposite direction
      const voucherEntries = voucher.entries.filter(entry => {
        if (entry.id === paymentEntry.id) return false;
  ...
  // Update mutation — uses bulk endpoint that replaces all entries atomically
  ...
  const onSubmitPaymentReceipt = (data: VoucherFormData) => {
    ...
    // Source (payment) account entry
    // Payment non-liability: CR source (money leaves). Receipt non-liability: DR source (money arrives).
    // Payment liability: DR source (reduce liability). Receipt liability: CR source (increase liability).
    const paymentEntry = {
      ...
      debitAmount:
        (voucherType === "Receipt" && !isLiabilityPayment) ||
        (voucherType === "Payment" && isLiabilityPayment)
          ? total
          : "0",
      creditAmount:
        (voucherType === "Payment" && !isLiabilityPayment) ||
    ...
    updateMutation.mutate({ voucherUpdates, entries: [paymentEntry, ...contraEntries] });
  };
  ...
  if (isStockTransfer && voucher.transferData) {
    ...
    const exportToExcel = async () => {
      ...
      const exportData = transferItems.map(item => ({
        ...
        'Moto Name': item.stockItemName || 'Unknown',
