...
export default function WarrantyPage() {
  ...
  const { data: warranties = [], isLoading } = useQuery<Warranty[]>({
    queryKey: [`/api/warranties/customer/${selectedCustomerId}`, selectedCompany?.id, selectedCustomerId],
  ...
  const createMutation = useMutation({
    mutationFn: async (data: WarrantyFormValues) => {
      return await apiRequest("POST", "/api/warranties", {
        ...data,
        companyId: selectedCompany?.id,
        customerId: selectedCustomerId,
      });
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/warranties/customer/${selectedCustomerId}`, selectedCompany?.id, selectedCustomerId] });
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: WarrantyFormValues & { id: number }) => {
      return await apiRequest("PUT", `/api/warranties/${data.id}`, data);
    },
    onSuccess: () => {
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedCustomerId && (
              <span className="text-sm text-muted-foreground">
                Viewing warranties for: <span className="font-medium">{getCustomerName(selectedCustomerId)}</span>
              </span>
            )}
          </div>
                    ...
                    <TableBody>
                      {warranties.map((warranty) => (
                        <TableRow key={warranty.id} data-testid={`row-warranty-${warranty.id}`}>
                          ...
                          <TableCell className="max-w-[150px] truncate">{warranty.voidReason || "-"}</TableCell>
                          <TableCell className="max-w-[150px] truncate">{warranty.notes || "-"}</TableCell>
                          <TableCell>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Enter reason for voiding warranty" {...field} data-testid="input-void-reason" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit" disabled={createMutation.isPending || updateMutation.isPending} data-testid="button-save-warranty">
