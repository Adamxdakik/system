  ...
  import { Plus, Edit, Building2, Users, ChevronDown, ChevronUp, Trash2, CalendarRange, Settings2, Wrench, MapPin, ChevronRight, Bot, MessageCircle, RefreshCw, Calculator, Loader2, Shield, Activity, Database, HardDrive, LogOut, Globe, Eye, Clock } from "lucide-react";
  ...
  export default function Settings() {
    ...
    const { data: activeSessions = [], isLoading: isLoadingSessions, refetch: refetchSessions } = useQuery<any[]>({
    ...
    // Admin: DB stats
    const { data: dbStats, isLoading: isLoadingDbStats, refetch: refetchDbStats } = useQuery<any>({
    ...
    const forceLogoutMutation = useMutation({
      ...
      onError: (error: any) => {
        toast({ title: "Error", description: error.message || "Failed to terminate session", variant: "destructive" });
      },
    });
    ...
    const toggleEmployeeAccessMutation = useMutation({
      mutationFn: async ({ userId, enabled }: { userId: string; enabled: boolean }) => {
        const res = await apiRequest("PATCH", `/api/users/${userId}`, { employeeInventoryAccess: enabled });
        return await res.json();
      },
      onSuccess: () => {
        toast({
          title: "Success",
          description: "Employee inventory access updated",
        });
        queryClient.invalidateQueries({ queryKey: ["/api/users"] });
      },
      onError: (error: any) => {
        ...
        <Tabs defaultValue="companies" className="space-y-6">
          <TabsList data-testid="tabs-settings">
            ...
            {currentUser?.role === "Admin" && (
              <TabsTrigger value="admin-panel" data-testid="tab-admin-panel">
                            ...
                            <div className="flex items-center gap-2">
                              <Switch
                                ...
                                onCheckedChange={(checked) =>
                                  toggleEmployeeAccessMutation.mutate({ userId: user.id, enabled: checked })
                                }
          ...
          <TabsContent value="active-users" className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-1">Monitor currently active users and their location in the application.</p>
              </div>
                    ...
                    <TableBody>
                      {activeUsersList.map((u: any) => (
                        <TableRow key={u.userId}>
                          ...
                          <TableCell className="font-mono text-sm text-muted-foreground">{u.currentPage}</TableCell>
          ...
          <TabsContent value="login-history" className="space-y-4">
                  ...
                  <TableBody>
                    {loginHistoryList.map((row: any) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="font-mono text-sm text-muted-foreground">{row.ipAddress || "–"}</TableCell>
                        <TableCell className="text-sm text-muted-foreground max-w-xs truncate">{row.userAgent || "–"}</TableCell>
                        <TableCell className="text-right text-sm text-muted-foreground">
                          {row.createdAt
            ...
            <p className="text-xs text-muted-foreground">
              Showing last {loginHistoryList.length} login events. Location data is approximate and based on IP address.
            </p>
          </TabsContent>
            ...
            <TabsContent value="admin-panel" className="space-y-6">
                ...
                <Card className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{dbStats?.total?.total_size ?? "—"}</p>
                    </div>
                  </div>
                </Card>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Card className="p-0 overflow-hidden">
                  {isLoadingDbStats ? (
                    ...
                    <div className="p-6 text-muted-foreground text-sm">No data available.</div>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {(dbStats.tables as any[]).map((t: any) => (
                          <TableRow key={t.table_name}>
                            <TableCell className="font-mono text-sm">{t.table_name}</TableCell>
                            <TableCell className="text-right text-sm">{Number(t.estimated_rows).toLocaleString()}</TableCell>
                            <TableCell className="text-right text-sm font-medium">{t.size}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                  ...
                  <Link href="/deleted-items">
                    <Card className="p-4 hover-elevate cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm">Deleted Items</p>
                          <p className="text-xs text-muted-foreground">View and restore deleted records</p>
                        </div>
                  ...
                  <Link href="/orphaned-records">
                    <Card className="p-4 hover-elevate cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm">Orphaned Records</p>
                          <p className="text-xs text-muted-foreground">Find records with deleted locations</p>
                        </div>
                        <ChevronRight className="h-4 w-4 text-muted-foreground ml-auto" />
                      </div>
                    </Card>
                  ...
                  <Link href="/chatbot-settings">
                    <Card className="p-4 hover-elevate cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm">AI Chatbot Settings</p>
                          <p className="text-xs text-muted-foreground">Manage AI assistant access</p>
                        </div>
                        <ChevronRight className="h-4 w-4 text-muted-foreground ml-auto" />
                      </div>
                    </Card>
                  ...
                  <Card className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="font-medium text-sm">Initialize Accounting Balances</p>
                        <p className="text-xs text-muted-foreground">Create Owner's Capital accounts</p>
                      </div>
                      <Button
                        size="sm"
                        onClick={() => {
              ...
              <div className="space-y-3">
                ...
                <Card className="p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="font-medium text-sm">Invalidate All Client Caches</p>
                      <p className="text-xs text-muted-foreground">Force all queries to refetch fresh data from the server</p>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        ...
                        toast({ title: "Cache Cleared", description: "All client caches have been invalidated." });
                      }}
                    >
                      <RefreshCw className="h-4 w-4 mr-2" />
                      Clear Cache
