        ...
        <div className="space-y-5 py-2">
          <div className="space-y-2">
            ...
            >
              <option value="">
                {isLoading
                  ? "Loading completed output..."
                  : "Select completed Final Product output"}
              </option>
              {outputs.map((output) => (
                <option key={output.id} value={output.id}>
                  {output.stockItemName || `Model #${output.stockItemId}`} ·{" "}
                  {output.locationName || "No location"} · {output.remainingUnits} remaining ·{" "}
