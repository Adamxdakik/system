...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/users/heartbeat", requireAuth, (req, res) => {
    ...
    activeUsers.set(user.id, {
      userId: user.id,
      username: user.username,
      role: (req.session as any).currentRole || "User",
  ...
  // Employee Inventory - Get all inventory across all locations WITHOUT cost data
  app.get("/api/employee-inventory", requireAuth, async (req, res) => {
    try {
      if (!req.session.currentCompanyId) {
        return res.status(400).json({ message: "No company selected" });
      }

      // Check if user has employee inventory access
      ...
      const sanitizedInventory = inventory
        .filter((item: any) => parseFloat(item.quantity || "0") !== 0)
        .map((item: any) => ({
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await storage.updateCustomer(customerId, { deletedAt: new Date() });
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await storage.updateBikePurchase(purchaseId, { deletedAt: new Date() } as any);
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await storage.updatePartPurchase(purchaseId, { deletedAt: new Date() } as any);
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await storage.updateServiceHistory(recordId, { deletedAt: new Date() } as any);
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const warranties = await storage.getAllWarranties(req.session.currentCompanyId);
        res.json(warranties);
      } catch (error: any) {
        res.status(500).json({ message: error.message });
      }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const warranties = await storage.getWarrantiesByCustomer(customerId, req.session.currentCompanyId);
        res.json(warranties);
      } catch (error: any) {
        res.status(500).json({ message: error.message });
      }
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await storage.updateWarranty(warrantyId, { deletedAt: new Date() } as any);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const log = await storage.createCommunicationLog(parsed);
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const log = await storage.updateCommunicationLog(logId, parsed);
        res.json(log);
      } catch (error: any) {
        res.status(400).json({ message: error.message });
      }
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        await storage.updateCommunicationLog(logId, { deletedAt: new Date() } as any);
      ...
      try {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (chargeType && chargeAmount) {
            ...
            // Skip invalid rows silently during parse phase (validation happens in validate step)
  ...
  // Add container item
  app.post("/api/containers/:id/items", requireAuth, requireNonPOS, async (req, res) => {
    try {
      const containerId = parseInt(req.params.id);
      const container = await storage.getContainerById(containerId);
      if (!container) {
  ...
  app.patch("/api/containers/:id/items/:itemId", requireAuth, requireNonPOS, async (req, res) => {
    try {
      const containerId = parseInt(req.params.id);
  ...
  app.delete("/api/containers/:id/items/:itemId", requireAuth, requireNonPOS, async (req, res) => {
    try {
      const containerId = parseInt(req.params.id);
      const itemId = parseInt(req.params.itemId);
  ...
  // Add/update container charges (freight, etc.)
  app.post("/api/containers/:id/charges", requireAuth, requireNonPOS, async (req, res) => {
    try {
      const containerId = parseInt(req.params.id);
      const container = await storage.getContainerById(containerId);
      if (!container) {
      ...
      const { charges } = req.body; // Array of { chargeType, amount }
      
      // Delete existing charges and replace with new ones
  ...
  // Create purchase voucher for manual container import (credits supplier balance)
  // This endpoint is idempotent - calling it multiple times will only create one voucher
  app.post("/api/containers/:id/create-purchase-voucher", requireAuth, requireNonPOS, async (req, res) => {
    try {
      const containerId = parseInt(req.params.id);
      const companyId = req.session.currentCompanyId;
      ...
      // Deterministic voucher number based on container ID for idempotency
      const voucherNumber = `MCONT-${containerId}`;

      // Check if a purchase voucher already exists for this container (idempotency check)
      const existingVouchers = await db
        ...
        .where(
          and(
            ...
            eq(vouchers.voucherType, "Purchase")
          )
        )
        .limit(1);

      if (existingVouchers.length > 0) {
        // Voucher already exists, skip creation (idempotent response)
        return res.json({ success: true, message: "Purchase voucher already exists", voucherId: existingVouchers[0].id });
      }

      // Get container items and charges
      const items = await storage.getContainerItems(containerId);
      const charges = await storage.getChargesByContainer(containerId);

      // Calculate totals - only items + charges that are owed to supplier
      // Following PO import pattern: grand total (items + charges) is credited to supplier
      const itemsTotal = items.reduce((sum, i) => sum + parseFloat(i.lineTotal), 0);
      const chargesTotal = charges.reduce((sum, c) => sum + parseFloat(c.amount), 0);
      const grandTotal = itemsTotal + chargesTotal;

      if (grandTotal <= 0) {
        // No items means nothing to credit, return success without creating voucher
      ...
      // Create purchase voucher with deterministic number
      const voucherDate = container.importDate || new Date().toISOString().split('T')[0];
      ...
      // Credit: Supplier account (Accounts Payable increases - this updates supplier balance)
      await storage.createVoucherEntry({
        voucherId: voucher.id,
          ...
          if (existingOffload) {
            ...
            for (const po of pos) {
              ...
              for (const item of lineItems) {
                // Skip items with null/undefined/zero stockItemId to prevent SQL syntax errors
            ...
            // Also reverse manual container items
            const manualItems = await storage.getContainerItems(containerId);
            for (const item of manualItems) {
              // Skip items with null/undefined/zero stockItemId
              if (!item.stockItemId || item.stockItemId === 0) {
          ...
          for (const item of manualItems) {
            ...
            if (inv) {
              ...
              if (newQty <= 0) {
                ...
                await tx.update(inventory).set({ quantity: newQty.toString() }).where(eq(inventory.id, inv.id));
              }
            }
          }
  ...
  // Update container tracking info (manual entry)
  app.patch(
  ...
  // Fetch container tracking from TrackingMore API
  app.post(
  ...
  app.get("/api/vouchers", requireAuth, async (req, res) => {
    try {
      ...
      // Optionally hide vouchers auto-generated by sales/purchase/stock-transfer/
      // production/consumption/closing flows. Daybook calls this with
      ...
      const finalVouchers =
        includeSystem === "false"
          ? sanitizedVouchers.filter(
              (v: any) => !SYSTEM_VOUCHER_TYPES.has(v.voucherType),
            )
          : sanitizedVouchers;
  ...
  // ───────────────────────────────────────────────────────────────────────
  // Simplified accounting endpoints for users who don't think in debit/credit.
  // Both endpoints write balanced double-entry vouchers under the hood so the
  // Trial Balance / Balance Sheet / Income Statement keep working perfectly;
  // the user just sees "From → To" or "Adjust this account ±X".
  //
  // /api/accounts/transfer — pay/receive between two real accounts.
  // /api/accounts/adjust    — bump one account up/down; opposite leg is
  //                           silently routed to a per-tenant suspense ledger
  //                           "Manual Adjustments" so books stay balanced.
  // ───────────────────────────────────────────────────────────────────────

  // Map account "kind" + ledger.accountType → natural balance side.
  // Used to translate the user's "increase / decrease" intent into the
  // correct debit-or-credit posting on the target account.
  function naturalSideFor(
    kind: "ledger" | "bank" | "fixedAsset" | "supplier" | "employee",
    accountType?: string | null,
  ): "Dr" | "Cr" {
    if (kind === "bank" || kind === "fixedAsset") return "Dr";
    if (kind === "supplier" || kind === "employee") return "Cr";
    const debitNatural = new Set([
      "Asset",
      "Bank",
      "Cash",
      "Expense",
  ...
  // Lazily fetch (or create) the per-tenant "Manual Adjustments" suspense
  // ledger so single-account adjustments still post a balanced voucher.
  // Type "Equity" so the plug lands in equity rather than skewing assets or
  // liabilities. Returns the ledger row.
  async function getOrCreateManualAdjustmentsAccount(companyId: number) {
    ...
    try {
      ...
      // Concurrent first-time call lost the race. Postgres unique-violation
      // is code 23505. Re-read the row that the other request inserted.
  ...
  // Build a voucherEntries insert payload for a given account reference.
  ...
  // Verify an account is owned by the caller's company before posting to it.
  // Returns { ok, accountType?, message? }.
  async function verifyAccountOwnership(
    kind: "ledger" | "bank" | "fixedAsset" | "supplier" | "employee",
    accountId: number,
    companyId: number,
  ): Promise<{ ok: boolean; accountType?: string | null; message?: string }> {
    if (kind === "ledger") {
      const [row] = await db
        ...
        .limit(1);
      return row
        ? { ok: true, accountType: row.accountType }
        : { ok: false, message: "Ledger account not found" };
    }
    if (kind === "bank") {
      const [row] = await db
        .select({ id: bankAccounts.id })
        ...
        .limit(1);
      return row ? { ok: true } : { ok: false, message: "Bank account not found" };
    }
    if (kind === "fixedAsset") {
      const [row] = await db
        .select({ id: fixedAssets.id })
        ...
        .limit(1);
      return row ? { ok: true } : { ok: false, message: "Fixed asset not found" };
    }
    if (kind === "supplier") {
      // Suppliers are global in this schema, so just confirm existence.
      const [row] = await db
        .select({ id: suppliers.id })
        .from(suppliers)
        .where(eq(suppliers.id, accountId))
        .limit(1);
      return row ? { ok: true } : { ok: false, message: "Supplier not found" };
    }
    // employee
    const [row] = await db
  ...
  // Body: { fromKind, fromId, toKind, toId, amount, date, notes? }
  // Creates one balanced "Payment"-type voucher: Debit destination, Credit source.
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const { fromKind, fromId, toKind, toId, amount, date, notes } = req.body ?? {};
        const validKinds = ["ledger", "bank", "fixedAsset", "supplier", "employee"] as const;
        ...
        const fromCheck = await verifyAccountOwnership(fromKind as Kind, fromIdNum, companyId);
        if (!fromCheck.ok) return res.status(404).json({ message: `From: ${fromCheck.message}` });
        const toCheck = await verifyAccountOwnership(toKind as Kind, toIdNum, companyId);
        ...
        // Wrap voucher header + both legs in a single DB transaction so we
        // can never persist a half-written, unbalanced voucher.
        const createdVoucher = await db.transaction(async (tx) => {
          ...
          await tx.insert(voucherEntries).values({
            voucherId: v.id,
            ...entryFieldsFor(toKind as Kind, toIdNum),
          ...
          await tx.insert(voucherEntries).values({
            voucherId: v.id,
            ...entryFieldsFor(fromKind as Kind, fromIdNum),
            debitAmount: "0",
        ...
        // Keep employee currentBalance cache in sync if either leg touched one
  ...
  // Body: { kind, accountId, direction: "increase"|"decrease", amount, date, notes? }
  // Posts a balanced 2-line "Journal" voucher: target account leg + opposite
  // leg to the per-tenant Manual Adjustments suspense ledger.
  app.post(
    "/api/accounts/adjust",
    requireAuth,
    requireNonPOS,
    async (req, res) => {
      try {
        ...
        const { kind, accountId, direction, amount, date, notes } = req.body ?? {};
        const validKinds = ["ledger", "bank", "fixedAsset", "supplier", "employee"] as const;
        type Kind = typeof validKinds[number];
        ...
        const ownership = await verifyAccountOwnership(kind as Kind, idNum, companyId);
        if (!ownership.ok) return res.status(404).json({ message: ownership.message });

        const naturalSide = naturalSideFor(kind as Kind, ownership.accountType);
        // increase pushes balance in its natural direction; decrease pushes it back.
        const sideForTarget: "Dr" | "Cr" =
          direction === "increase" ? naturalSide : naturalSide === "Dr" ? "Cr" : "Dr";

        const adjAccount = await getOrCreateManualAdjustmentsAccount(companyId);
        const voucherNumber = `ADJ-${Date.now()}`;
        const amountStr = amt.toFixed(2);

        // Atomic voucher header + both legs.
        const createdVoucher = await db.transaction(async (tx) => {
          const [v] = await tx
            .insert(vouchers)
            .values({
  ...
  // Get the container linked to a Purchase voucher (via its purchase order)
  app.get("/api/vouchers/:id/linked-container", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Dashboard drill-down: Daily/Weekly/Monthly sales and expenses with date filtering
  app.get("/api/stats/sales-expenses-drilldown", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      // Get all sales vouchers in the period
      const salesVouchers = await db
        .select()
        .from(vouchers)
        .where(
      ...
      // Calculate total sales and daily breakdown
      let totalSales = 0;
      ...
      // Get expense vouchers (Payment, Journal with expense accounts)
      ...
      const expenseAccounts = await db
        ...
        .where(
          and(
            eq(ledgerAccounts.companyId, companyId),
            inArray(ledgerAccounts.accountType, expenseAccountTypes),
          ),
        )
        .execute();
      const expenseAccountIds = expenseAccounts.map((a) => a.id);

      // Get all vouchers in period
      const periodVouchers = await db
        .select()
        .from(vouchers)
        .where(
          and(
      ...
      const expensesByDay = Array.from(dailyExpenses.entries())
        .map(([date, amount]) => ({ date, amount }))
        .sort((a, b) => a.date.localeCompare(b.date));
  ...
  // Dashboard metrics - total sales, gross profit, COGS, operating expenses, inventory, trend data
  app.get("/api/stats/dashboard-metrics", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      // 1. Sales vouchers in the date range
      ...
      // 2. Sales items for those vouchers, joined with stock groups for categorisation
      const salesItemsData: Array<{
        voucherId: number;
        quantity: string;
        totalSales: string;
        totalCost: string;
        stockGroupName: string | null;
      }> = salesVoucherIds.length > 0
        ? await db
            .select({
              voucherId: salesItems.voucherId,
              quantity: salesItems.quantity,
      ...
      for (const item of salesItemsData) {
        ...
        if (
          groupNameLower.includes("moto") ||
          groupNameLower.includes("bike") ||
          groupNameLower.includes("motorcycle")
        ) {
          motosSold += qty;
        } else {
          partsSold += qty;
      ...
      // 4. Operating expenses for the period (from expense-type ledger account entries)
      const periodVoucherIds: number[] = [];
      ...
      const allAccounts = await storage.getAllLedgerAccounts(companyId);
      const expenseAccountIds = new Set<number>(
        allAccounts
          .filter(
            acc =>
              ...
              acc.accountType === "Direct Expense" ||
              acc.accountType === "Indirect Expense" ||
              acc.accountType === "Operating Expenses"
          )
          .map(acc => acc.id)
      );
      ...
      // 5. Cash in hand - all-time balance across all cash-type accounts
      let cashInHand = 0;
      if (cashAccountIds.length > 0) {
        const allCompanyVouchers = await db
          .select({ id: vouchers.id })
          .from(vouchers)
          .where(and(eq(vouchers.companyId, companyId), eq(vouchers.optional, false)))
          .execute();
        ...
        if (allCompanyVoucherIds.length > 0) {
          const cashEntries = await db
            ...
            .where(
              and(
                inArray(voucherEntries.voucherId, allCompanyVoucherIds),
                inArray(voucherEntries.ledgerAccountId, cashAccountIds),
              )
            )
            .execute();
      ...
      // 6. Current inventory value, split into Motos vs Parts by stock group name
      // Only count active (non-deleted) locations to avoid double-counting
      ...
      const stockItemGroupRows = await db
      ...
      for (const inv of inventoryRows) {
        ...
        if (qty <= 0) continue;
        const value =
          parseFloat(inv.totalValue || "0") ||
          qty * parseFloat(inv.averageRate || "0");
      ...
      // 7. Trend data - aggregate sales by sub-period
      const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
      const trendMap = new Map<string, { sales: number; unitsSold: number; cost: number }>();
      ...
      // Build inventory trend: reconstruct historical inventory value per sub-period.
      // Current inventoryValue is the end-of-period snapshot.
      // Work forward: estimated inventory at start of period = inventoryValue + sum of all period COGS.
      // Then subtract each sub-period's COGS as we move through time.
      ...
      const trendData = trendEntries.map(([label, data]) => {
        ...
        return {
          label,
          sales: data.sales,
          unitsSold: Math.round(data.unitsSold * 1000) / 1000,
  ...
  // Income Statement - Monthly financial summary like Tally P&L
  app.get("/api/stats/income-statement", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      const selectedLocationId = locationId && locationId !== "all" ? parseInt(locationId as string) : null;
      ...
      // Get stock groups for revenue/COGS categorization
      const allStockGroups = await db
        .select()
        .from(stockGroups)
        .where(and(eq(stockGroups.companyId, companyId), isNull(stockGroups.deletedAt)));

      // Create maps for stock item to group mapping
      const allStockItems = await storage.getAllStockItems(companyId);
      const stockItemGroupMap = new Map<number, { groupId: number | null; groupName: string }>();
      for (const item of allStockItems) {
      ...
      // Get unique stock group names for revenue rows
      ...
      // Get all Sales vouchers for the year
      const yearStart = `${selectedYear}-01-01`;
      const yearEnd = `${selectedYear}-12-31`;

      const salesVouchersQuery = db
        .select()
        .from(vouchers)
        .where(
          and(
            ...
            selectedLocationId ? eq(vouchers.locationId, selectedLocationId) : sql`true`
          )
        );
      ...
      // Get all sales items for these vouchers
      const salesItemsData = salesVoucherIds.length > 0
        ? await db
            .select()
            .from(salesItems)
            .where(inArray(salesItems.voucherId, salesVoucherIds))
      ...
      // Create voucher date map
      const voucherDateMap = new Map(salesVouchers.map(v => [v.id, v.voucherDate]));

      // Initialize revenue and COGS data by group and month
      const revenueByGroupMonth = new Map<string, Map<string, number>>();
      const cogsByGroupMonth = new Map<string, Map<string, number>>();

      for (const groupName of uniqueGroups) {
        revenueByGroupMonth.set(groupName, new Map());
        cogsByGroupMonth.set(groupName, new Map());
      ...
      // Aggregate sales and COGS by stock group and month
      for (const item of salesItemsData) {
        const voucherDate = voucherDateMap.get(item.voucherId);
        if (!voucherDate) continue;
      ...
      // Get expense vouchers (Payment, Journal) and their entries
      const expenseAccountTypes = ["Expense", "Direct Expense", "Indirect Expense"];
      const allLedgerAccounts = await storage.getAllLedgerAccounts(companyId);
      const expenseAccounts = allLedgerAccounts.filter(acc =>
        expenseAccountTypes.includes(acc.accountType)
      );
      const expenseAccountIds = expenseAccounts.map(a => a.id);

      // Get all vouchers for the year (for expense entries)
      const periodVouchersQuery = db
        .select()
        .from(vouchers)
        .where(
          and(
      ...
      const expenseEntries = periodVoucherIds.length > 0 && expenseAccountIds.length > 0
        ? await db
            .select()
            .from(voucherEntries)
            .where(
              and(
                inArray(voucherEntries.voucherId, periodVoucherIds),
                inArray(voucherEntries.ledgerAccountId, expenseAccountIds)
              )
            )
        : [];

      // Group expenses by account name and month
      const expenseByAccountMonth = new Map<string, Map<string, number>>();
      ...
      for (const entry of expenseEntries) {
        ...
        if (!expenseByAccountMonth.has(accountName)) {
          expenseByAccountMonth.set(accountName, new Map());
      ...
      // Get bank account movements (MONEY OUT = withdrawals)
      const bankAccounts = allLedgerAccounts.filter(acc =>
        acc.accountType === "Bank" || acc.accountType === "Cash"
      );
      const bankAccountIds = bankAccounts.map(a => a.id);

      const bankMovementsByMonth = new Map<string, number>();
      ...
      if (periodVoucherIds.length > 0 && bankAccountIds.length > 0) {
        const bankEntries = await db
          ...
          .where(
            and(
              inArray(voucherEntries.voucherId, periodVoucherIds),
              inArray(voucherEntries.ledgerAccountId, bankAccountIds)
            )
          );
        ...
        for (const entry of bankEntries) {
          ...
          const date = new Date(voucherDate);
          const monthKey = months[date.getMonth()]?.key;
          if (!monthKey) continue;

          // Credits to bank accounts are money going out
      ...
      // Get interest/loan accounts for SOFICOM INTEREST style row
      const loanAccounts = allLedgerAccounts.filter(acc =>
        acc.accountType === "Loans" ||
        acc.name.toLowerCase().includes("interest") ||
        acc.name.toLowerCase().includes("soficom")
      );
      ...
      if (periodVoucherIds.length > 0 && loanAccountIds.length > 0) {
        const interestEntries = await db
          ...
          .where(
            and(
              inArray(voucherEntries.voucherId, periodVoucherIds),
              inArray(voucherEntries.ledgerAccountId, loanAccountIds)
            )
          );
        ...
        for (const entry of interestEntries) {
          ...
          // Debits to interest/loan accounts
  ...
  // Liquidation History - daily snapshots of business value
  app.get("/api/stats/liquidation-history", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      const selectedLocationId = req.query.locationId ? parseInt(req.query.locationId as string) : null;
      ...
      // Categorize accounts
      const supplierAccounts = allLedgerAccounts.filter(acc =>
        acc.accountType === "Loans" ||
        acc.accountType === "Duty Agent" ||
        acc.accountType === "Transporter Agent" ||
        acc.accountType === "Accounts Payable" ||
        acc.name.toLowerCase().includes("soficom") ||
        acc.name.toLowerCase().includes("tvs")
      );

      const cashBankAccounts = allLedgerAccounts.filter(acc =>
      ...
      // Get all vouchers for the year with entries
      const yearVouchers = await db
        .select()
        .from(vouchers)
        .where(
          and(
      ...
      // Get all voucher entries
      const allEntries = voucherIds.length > 0
        ? await db.select().from(voucherEntries).where(inArray(voucherEntries.voucherId, voucherIds))
        : [];

      // Get current stock values by stock group (using inventory table)
      const inventoryData = await db
        .select({
          stockItemId: inventory.stockItemId,
          quantity: inventory.quantity,
      ...
      // Calculate current stock values by group
      const currentStockByGroup = new Map<string, number>();
      ...
      for (const entry of allEntries) {
        const date = voucherDateMap.get(entry.voucherId);
        if (!date) continue;
        if (!entriesByDate.has(date)) entriesByDate.set(date, []);
        entriesByDate.get(date)!.push(entry);
      }

      // Get unique dates that have activity
      ...
      for (const date of sortedDates) {
        // Process ledger entries for this date
        ...
        for (const entry of dateEntries) {
          ...
          // Update supplier balances (credits increase liability)
          ...
          // Update cash balances (debits increase asset, credits decrease)
          if (cashBankAccounts.some(a => a.id === entry.ledgerAccountId)) {
            ...
            // Track money out (credits to cash/bank accounts)
            if (credit > 0) {
        ...
        // Use current stock values by group (same for all dates - current snapshot)
        const stockValuesList = Array.from(currentStockByGroup.entries())
        ...
        // Calculate IN POCKET (sum of cash positions)
        ...
        // Calculate total supplier balance (liability)
        const totalSupplierBalance = supplierBalancesList.reduce((sum, s) => sum + s.balance, 0);

        // Liquidation Value = Stock Value + Cash - Supplier Balances
      ...
      // Build column names from all entries
      const supplierNamesSet = new Set<string>();
      const stockGroupNamesSet = new Set<string>();
      const cashAccountNamesSet = new Set<string>();
  ...
  // Liquidation Value - snapshot of current assets vs liabilities
  app.get("/api/stats/liquidation-value", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) return res.status(400).json({ message: "No company selected" });

      // Helper: calculate running balance for a set of ledger account IDs
      const calcAccountBalance = async (accounts: any[], isLiability: boolean) => {
        const results: { name: string; balance: number }[] = [];
        for (const account of accounts) {
          const entries = await db
            .select({ credit: voucherEntries.creditAmount, debit: voucherEntries.debitAmount })
            .from(voucherEntries)
            .innerJoin(vouchers, eq(voucherEntries.voucherId, vouchers.id))
            .where(and(
              eq(voucherEntries.ledgerAccountId, account.id),
          ...
          const signedOpening = isLiability
            ? (openingSide === "Cr" ? openingRaw : -openingRaw)
            : (openingSide === "Dr" ? openingRaw : -openingRaw);
      ...
      let motoValue = 0; let motoQty = 0;
      let partsValue = 0; let partsQty = 0;
      for (const row of inventoryData) {
        const val = parseFloat(row.totalValue || "0");
        const qty = parseFloat(row.quantity || "0");
        if (row.allocateImportCosts) { motoValue += val; motoQty += qty; }
        else { partsValue += val; partsQty += qty; }
      }

      // 2. Containers OTW
      const otwContainers = await db
        .select({ id: containers.id, name: containers.containerNumber, grandTotal: containers.grandTotal })
        .from(containers)
      ...
      // 3. Fixed assets — sum purchaseAmount (book value)
      const allFixedAssets = await storage.getAllFixedAssets(companyId);
      const fixedAssetItems = allFixedAssets
        .filter((a: any) => a.active)
        .map((a: any) => ({ name: a.name, balance: parseFloat(a.purchaseAmount || "0") }));
      const fixedAssetsTotal = fixedAssetItems.reduce((s: number, a: { balance: number }) => s + a.balance, 0);

      // 4. Cash + Bank accounts
      const cashBankAccs = await db.select().from(ledgerAccounts).where(and(
        eq(ledgerAccounts.companyId, companyId), isNull(ledgerAccounts.deletedAt),
        sql`${ledgerAccounts.accountType} IN ('Cash', 'Bank')`
      ));
      ...
      const suppliersByLedger = await db.select().from(ledgerAccounts).where(and(
        eq(ledgerAccounts.companyId, companyId), isNull(ledgerAccounts.deletedAt),
        eq(ledgerAccounts.accountType, "Accounts Payable")
      ));
      const supplierLedgerItems = await calcAccountBalance(suppliersByLedger, true);
      // Also include raw supplier entries (those not linked to a ledger account)
      const supplierRawBalance = supplierEntries.reduce((s, e) => s + parseFloat(e.credit || "0") - parseFloat(e.debit || "0"), 0);
      const allSupplierItems = supplierLedgerItems.length > 0
        ? supplierLedgerItems
        : (supplierRawBalance !== 0 ? [{ name: "Suppliers", balance: supplierRawBalance }] : []);
      const suppliersTotal = allSupplierItems.reduce((s, a) => s + a.balance, 0);

      // 6. Duty Agents
      const dutyAgentAccs = await db.select().from(ledgerAccounts).where(and(
        eq(ledgerAccounts.companyId, companyId), isNull(ledgerAccounts.deletedAt),
        eq(ledgerAccounts.accountType, "Duty Agent")
      ));
      ...
      // 7. Transporters
      const transporterAccs = await db.select().from(ledgerAccounts).where(and(
        eq(ledgerAccounts.companyId, companyId), isNull(ledgerAccounts.deletedAt),
        eq(ledgerAccounts.accountType, "Transporter Agent")
      ));
      ...
      // 8. Loans
      const loanAccs = await db.select().from(ledgerAccounts).where(and(
        eq(ledgerAccounts.companyId, companyId), isNull(ledgerAccounts.deletedAt),
  ...
  app.get("/api/location-summary", requireAuth, async (req, res) => {
    try {
      ...
      const inventoryMap = new Map<string, { quantity: number; rate: number; value: number; color: string | null; assignedStatus: string | null }>();
      for (const inv of inventoryData) {
      ...
      const result: Array<{
        ...
        code: string;
        name: string;
        locationData: Record<number, { quantity: number; rate: number; value: number; color: string | null; assignedStatus: string | null }>;
        items: Array<{
          id: number;
          code: string;
          name: string;
      ...
      for (const group of allStockGroups) {
        ...
        const groupLocationData: Record<number, { quantity: number; rate: number; value: number; color: string | null; assignedStatus: string | null }> = {};
        ...
        for (const item of groupItems) {
          const itemLocationData: Record<number, { quantity: number; rate: number; value: number; color: string | null; assignedStatus: string | null }> = {};
      ...
      if (ungroupedItems.length > 0) {
        const ungroupedLocationData: Record<number, { quantity: number; rate: number; value: number; color: string | null; assignedStatus: string | null }> = {};
        for (const locId of locationIds) {
  ...
  // Update inventory color and assignedStatus
  const updateInventoryFieldsSchema = z.object({
    color: z.string().max(50).transform(v => v?.trim() || null).optional(),
    assignedStatus: z.string().max(100).transform(v => v?.trim() || null).optional(),
  });

  app.patch("/api/inventory/:locationId/:stockItemId", requireAuth, async (req, res) => {
    try {
      const locationId = parseInt(req.params.locationId);
      const stockItemId = parseInt(req.params.stockItemId);
      ...
      if (assignedStatus !== undefined) updates.assignedStatus = assignedStatus;
  ...
  // =========================================
  // MOTO ASSEMBLY ROUTES
  // =========================================

  // Get all moto assemblies for the current company
  app.get("/api/moto-assemblies", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      let assemblies = await db.select().from(motoAssemblies)
        .where(eq(motoAssemblies.companyId, companyId))
        .orderBy(desc(motoAssemblies.createdAt));
  ...
  // Get a single moto assembly with its parts
  app.get("/api/moto-assemblies/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
      ...
      const [assembly] = await db.select().from(motoAssemblies)
        .where(eq(motoAssemblies.id, id));

      if (!assembly) {
        return res.status(404).json({ message: "Assembly not found" });
      }

      // Verify belongs to current company
      if (assembly.companyId !== req.session.currentCompanyId) {
        return res.status(403).json({ message: "Access denied" });
      }

      // Get all parts for this assembly with stock item details
      const parts = await db.select({
        part: motoAssemblyParts,
        stockItem: stockItems,
      }).from(motoAssemblyParts)
        .leftJoin(stockItems, eq(motoAssemblyParts.stockItemId, stockItems.id))
        .where(eq(motoAssemblyParts.motoAssemblyId, id));
  ...
  // Create a new moto assembly
  app.post("/api/moto-assemblies", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      // Remove undefined assemblyCode from parsed before spreading
  ...
  // Update a moto assembly
  app.patch("/api/moto-assemblies/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid assembly ID" });
      ...
      const [existing] = await db.select().from(motoAssemblies)
        .where(eq(motoAssemblies.id, id));
      ...
      if (req.body.vin !== undefined) updates.vin = req.body.vin;
      if (req.body.motoModel !== undefined) updates.motoModel = req.body.motoModel;
      if (req.body.status !== undefined) updates.status = req.body.status;
      if (req.body.notes !== undefined) updates.notes = req.body.notes;
      if (req.body.completedDate !== undefined) updates.completedDate = req.body.completedDate;
      ...
      const [updated] = await db.update(motoAssemblies)
        .set(updates)
        .where(eq(motoAssemblies.id, id))
        .returning();
  ...
  // Delete a moto assembly
  app.delete("/api/moto-assemblies/:id", requireAuth, requireRole("Admin"), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid assembly ID" });
      ...
      // Delete parts first
      await db.delete(motoAssemblyParts)
        .where(eq(motoAssemblyParts.motoAssemblyId, id));

      // Delete assembly
      await db.delete(motoAssemblies)
        .where(eq(motoAssemblies.id, id));
  ...
  app.post("/api/moto-assemblies/:id/parts", requireAuth, async (req, res) => {
    try {
      ...
      const [assembly] = await db.select().from(motoAssemblies)
        .where(eq(motoAssemblies.id, assemblyId));
      ...
      await db.update(motoAssemblies)
        .set({
  ...
  app.delete("/api/moto-assembly-parts/:id", requireAuth, async (req, res) => {
    try {
      ...
      const [part] = await db.select().from(motoAssemblyParts)
        .where(eq(motoAssemblyParts.id, partId));

      if (!part) {
        return res.status(404).json({ message: "Part not found" });
      }

      // Get the assembly to verify ownership
      const [assembly] = await db.select().from(motoAssemblies)
        .where(eq(motoAssemblies.id, part.motoAssemblyId));
      ...
      await db.update(motoAssemblies)
        ...
        .where(eq(motoAssemblies.id, assembly.id));
  ...
  // =========================================
  // ASSEMBLY INVENTORY ROUTES (New Stage-Based System)
  // =========================================

  // Get assembly inventory for a location
  app.get("/api/assembly-inventory/:locationId", requireAuth, async (req, res) => {
    try {
      const locationId = parseInt(req.params.locationId);
      if (isNaN(locationId)) {
        return res.status(400).json({ message: "Invalid location ID" });
      ...
      // Get all inventory with stock item details
      const inventory = await db.select({
        inventory: assemblyInventory,
        stockItem: stockItems,
      })
        .from(assemblyInventory)
        .leftJoin(stockItems, eq(assemblyInventory.stockItemId, stockItems.id))
        .where(and(
          eq(assemblyInventory.locationId, locationId),
          eq(assemblyInventory.companyId, companyId)
        ))
  ...
  // Add a stock item to a stage (create inventory record)
  app.post("/api/assembly-inventory", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      const [existing] = await db.select().from(assemblyInventory)
        .where(and(
          eq(assemblyInventory.locationId, locationId),
          eq(assemblyInventory.stockItemId, stockItemId),
          eq(assemblyInventory.stage, stage)
        ));
      ...
      // Get stock item name for history
      const [stockItem] = await db.select().from(stockItems)
        .where(eq(stockItems.id, stockItemId));
  ...
  // Update qty for an assembly inventory record (Save button)
  app.patch("/api/assembly-inventory/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
      ...
      const [existing] = await db.select().from(assemblyInventory)
        .where(eq(assemblyInventory.id, id));
      ...
      // Get stock item name for history
      const [stockItem] = await db.select().from(stockItems)
        .where(eq(stockItems.id, existing.stockItemId));

      const [updated] = await db.update(assemblyInventory)
  ...
  app.post("/api/assembly-inventory/transfer", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      // Get source record
      const [sourceRecord] = await db.select().from(assemblyInventory)
        .where(and(
          eq(assemblyInventory.locationId, locationId),
          eq(assemblyInventory.stockItemId, stockItemId),
          eq(assemblyInventory.stage, fromStage),
          eq(assemblyInventory.companyId, companyId)
        ));
      ...
      // Subtract from source
      await db.update(assemblyInventory)
        .set({ qty: sourceRecord.qty - transferQty, updatedAt: new Date() })
        .where(eq(assemblyInventory.id, sourceRecord.id));

      // Find or create destination record
      const [destRecord] = await db.select().from(assemblyInventory)
        .where(and(
          eq(assemblyInventory.locationId, locationId),
          eq(assemblyInventory.stockItemId, stockItemId),
          eq(assemblyInventory.stage, toStage),
          eq(assemblyInventory.companyId, companyId)
        ));

      if (destRecord) {
        await db.update(assemblyInventory)
          .set({ qty: destRecord.qty + transferQty, updatedAt: new Date() })
          .where(eq(assemblyInventory.id, destRecord.id));
      } else {
  ...
  // Delete an assembly inventory record
  app.delete("/api/assembly-inventory/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid inventory ID" });
      ...
      await db.delete(assemblyInventory).where(eq(assemblyInventory.id, id));
  ...
  // Get assembly history for a location
  // Get all assembly history for company (for History page)
  app.get("/api/assembly-history", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      const history = await db.select().from(assemblyHistory)
        .where(eq(assemblyHistory.companyId, companyId))
        .orderBy(desc(assemblyHistory.createdAt))
        .limit(500);
  ...
  app.get("/api/assembly-history/:locationId", requireAuth, async (req, res) => {
    try {
      const locationId = parseInt(req.params.locationId);
      if (isNaN(locationId)) {
        return res.status(400).json({ message: "Invalid location ID" });
      ...
      const history = await db.select().from(assemblyHistory)
        .where(and(
          eq(assemblyHistory.locationId, locationId),
          eq(assemblyHistory.companyId, companyId)
        ))
        .orderBy(desc(assemblyHistory.createdAt))
        .limit(500);
  ...
  // Update assembly history record (editable fields: technician, completed, status)
  const updateAssemblyHistorySchema = z.object({
    technician: z.string().optional(),
    completed: z.boolean().optional(),
    status: z.enum(["pending", "completed"]).optional(),
  });

  app.patch("/api/assembly-history/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
      ...
      const [updated] = await db.update(assemblyHistory)
        .set(updates)
        .where(and(
          eq(assemblyHistory.id, id),
          eq(assemblyHistory.companyId, companyId)
        ))
        .returning();
  ...
  // Get all assembly tasks
  app.get("/api/assembly-tasks", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      const tasks = await db.select().from(assemblyTasks)
        .where(eq(assemblyTasks.companyId, companyId))
        .orderBy(desc(assemblyTasks.createdAt));
  ...
  app.post("/api/assembly-tasks", requireAuth, async (req, res) => {
    try {
      const companyId = req.session.currentCompanyId;
      if (!companyId) {
        return res.status(400).json({ message: "No company selected" });
      ...
      const { stockItemId, technician, user, action, details, qty, date } = req.body;
  ...
  // Update assembly task (completed toggle)
  app.patch("/api/assembly-tasks/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
      ...
      // Scope update to company to prevent cross-company updates
      const [updated] = await db.update(assemblyTasks)
        .set(updates)
        .where(and(
          eq(assemblyTasks.id, id),
          eq(assemblyTasks.companyId, companyId)
        ))
        .returning();
  ...
  app.get("/api/admin/active-sessions", requireAuth, requireRole("Admin"), async (req, res) => {
    try {
      ...
      const userMap = new Map((users.rows as any[]).map((u) => [u.id, u.username]));
      const companies = await db.execute(sql`SELECT id, name FROM companies`);
      const companyMap = new Map((companies.rows as any[]).map((c) => [c.id, c.name]));
  ...
  app.delete("/api/admin/sessions/:sid", requireAuth, requireRole("Admin"), async (req, res) => {
    try {
  ...
  // ─── Admin: DB storage stats ─────────────────────────────────────────────────
  app.get("/api/admin/db-stats", requireAuth, requireRole("Admin"), async (req, res) => {
    try {
  ...
  app.get("/api/offloads/:id", requireAuth, async (req, res) => {
    try {
      ...
      // Get items from all purchase orders linked to this container
