...
export interface IStorage {
  ...
  deleteContainerCharges(containerId: number): Promise<void>;
  ...
  // Bike Purchases
  getAllBikePurchases(companyId: number): Promise<schema.BikePurchase[]>;
  ...
  getWarrantyById(id: number): Promise<schema.Warranty | undefined>;
  getWarrantiesByCustomer(customerId: number, companyId: number): Promise<schema.Warranty[]>;
  createWarranty(warranty: schema.InsertWarranty): Promise<schema.Warranty>;
  updateWarranty(id: number, updates: Partial<schema.InsertWarranty>): Promise<schema.Warranty>;
...
export class DbStorage implements IStorage {
  ...
  async deleteContainer(id: number): Promise<void> {
    ...
    // Delete manual container purchase voucher (MCONT-{containerId})
    // This is created when containers are added manually (not via Excel import)
    ...
    // Delete container items (manual entry items)
    await db.delete(schema.containerItems).where(eq(schema.containerItems.containerId, id));
  ...
  async createContainerItem(item: schema.InsertContainerItem): Promise<schema.ContainerItem> {
    const [created] = await db.insert(schema.containerItems).values(item).returning();
    return created;
  }
  ...
  async deleteContainerItem(id: number): Promise<void> {
    await db.delete(schema.containerItems).where(eq(schema.containerItems.id, id));
  }
  ...
  async offloadContainer(
    ...
    // Get manual container items (for containers without POs)
    const manualContainerItems = await this.getContainerItems(containerId);

    // Use the passed costAllocationGroupIds for determining which groups receive import costs
    // If no groups are passed (empty array), fall back to groups with allocateImportCosts=true in database
    let effectiveCostAllocationGroupIds = costAllocationGroupIds;
    if (effectiveCostAllocationGroupIds.length === 0) {
      const stockGroupsWithCostAllocation = await db
        .select({ id: schema.stockGroups.id })
    ...
    // Check if any groups have cost allocation enabled
    const hasCostAllocationGroups = costAllocationGroupIdSet.size > 0;

    // Helper to check if a stock item should have costs allocated
    // If no groups are marked, fall back to allocating to all items (original behavior)
    const shouldAllocateCosts = (stockItemId: number): boolean => {
      if (!hasCostAllocationGroups) {
        return true; // Fallback: allocate to all items if no groups marked
      }
    ...
    for (const item of manualContainerItems) {
      ...
      // Fallback: use ratePerKg * weightKg / quantity
