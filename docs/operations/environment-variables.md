...
| --- | --- | --- |
| `NODE_ENV` | Start command or platform | `npm start` sets this to `production`. It enables secure session cookies and production security behavior. |
...
| `RENDER_GIT_COMMIT` | Render-provided | Fallback deployment identifier when `BUILD_VERSION` is not supplied. |
