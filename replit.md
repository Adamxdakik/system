...
**Known limitation / follow-up not done**: the 12 existing regression tests are shallow (auth-bypass + SPA-fallback assertions only — no authenticated tenant-boundary tests). Adding cookie-auth-driven integration tests for the new `locationId` and `sourceCompanyId` 403 paths is left as a future improvement; for now those paths are guarded by code review + manual verification, and the storage tests + concurrency stress test cover the data layer.
